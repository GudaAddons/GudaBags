                ...
                elseif #items == 1 then
                    ...
                    local consolidatedItem = {
                        ...
                        itemData = {
                            ...
                            -- Carried so the "Item Level" sort still works on
                            -- grouped stacks; it is constant per itemID, so the
                            -- first item's value describes the whole group.
...
-- (ItemScanner:ScanSlot), including for cached characters, whose slot tables
-- come back out of SavedVariables intact. Reading them here instead of calling
-- GetItemInfo also lets them self-heal: GET_ITEM_INFO_RECEIVED re-runs the scan
-- and writes fresh data back, where a sort-key cache keyed by itemID would keep
-- serving the placeholder classID for the rest of the session.
